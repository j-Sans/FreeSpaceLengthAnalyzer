...
Parser::Parser(std::string filename, int framesToIgnore, int framesToRecord, int* depthPerSlice, double diameter, double polydispersity) {
	avgDiameter = diameter;
	deviation = polydispersity;
...
std::vector<Slice> Parser::getSlices(const std::vector<Point>& particles, int* thickness, double height) {
	...
	for (int particle = 0; particle < particles.size(); particle++) { // yz dimension slices
		...
		slicesX[(int)(particles[particle].x / *thickness)].back().diameter = boxMuller(avgDiameter, deviation);
	}
	...
	for (int particle = 0; particle < particles.size(); particle++) { // xz dimension slices
		...
		slicesY[(int)(particles[particle].y / *thickness)].back().diameter = boxMuller(avgDiameter, deviation);
	}
	...
	for (int particle = 0; particle < particles.size(); particle++) { // xy dimension slices
		...
		slicesZ[(int)(particles[particle].z / *thickness)].back().diameter = boxMuller(avgDiameter, deviation);
	}
...
std::string Parser::outputSingleSlice(const Slice& slice, std::string filename) {
	...
	std::string str = std::to_string(slice.size()) + " " + std::to_string(avgDiameter) + " " + std::to_string(boxSize) + " " + std::to_string(volumeFraction) + "\n"; // Write the number of particles in the slice
	...
	for (auto particle = slice.begin(); particle != slice.end(); particle++) {
		if (slice.direction == x) {
			...
			if (newPoint)*/ str = std::to_string(particle->y) + " " + std::to_string(particle->z) + " " + std::to_string(particle->depth) + " " + std::to_string(particle->diameter) + "\n";
			...
			if (newPoint)*/ str = std::to_string(particle->x) + " " + std::to_string(particle->z) + " " + std::to_string(particle->depth) + " " + std::to_string(particle->diameter) + "\n";
			...
			if (newPoint)*/ str = std::to_string(particle->x) + " " + std::to_string(particle->y) + " " + std::to_string(particle->depth) + " " + std::to_string(particle->diameter) + "\n";
		}
		file.write(str.c_str(), str.length());
	}
...
double Parser::boxMuller(double mm, double ss)	/* normal random variate generator */
{				        /* mean mm, standard deviation ss */
	...
	{
		do {
			xx1 = 2.0 * (double)rand() / (double)(RAND_MAX) - 1.0;
			xx2 = 2.0 * (double)rand() / (double)(RAND_MAX) - 1.0;
			ww = xx1 * xx1 + xx2 * xx2;
		} while ( ww >= 1.0 );
