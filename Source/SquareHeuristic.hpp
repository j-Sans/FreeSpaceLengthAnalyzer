...
private:
    ...
    double radius, boxSize, volumeFraction;
    std::string outputFilename = "squares.txt";
    
    double heuristic(const std::vector<Point>& circles, double radius, double boxSize, Point* exampleSquare, double boxLengthDecrement, int numTests, double startingLength);
    bool isContained(const Point& circle, double radius, Point square, double sideLength, double regionLength, bool advancedAlgorithm = true);
