...
std::string SquareHeuristic::outputSquares(double boxLengthDecrement, int numCases, int numTests, double startingLength, /*bool nonVisual,*/ bool test) {
    ...
    for (int a = 0; a < numCases; a++) {
        double sideLength = heuristic(circles, radius, boxSize, &square, boxLengthDecrement, numTests, startingLength);
        if (!test) {
...
double SquareHeuristic::getRadius() {
    return radius;
}
 ...
 * @param radius The radius of the particles, as a double.
 *
 * @return The largest square length, as a double.
 */
double SquareHeuristic::heuristic(const std::vector<Point>& circles, double radius, double boxSize, Point* square, double boxLengthDecrement, int numTests, double startingLength) {
    ...
    for (; squareLength > 0; squareLength -= boxLengthDecrement) {
...
#ifdef NUM_THREADS
        auto trialFunc = [](std::vector<Point> circles, double radius, double boxSize, Point* square, double numTests, int* modeNumParticles) {
            ...
            for (int trial = 0; trial < numTests / (NUM_THREADS + 1.0); trial++) {
                *square = Point(boxSize * (double)rand() / (double)RAND_MAX, boxSize * (double)rand() / (double)RAND_MAX);
                ...
                for (auto circle = circles.begin(); circle != circles.end(); circle++) {
                    if (isContained(*circle, radius, *square, squareLength, boxSize)) numCircles++;
                }
        ...
        for (int a = 0; a < NUM_THREADS; a++) {
            threads.push_back(std::thread(trialFunc, circles, radius, boxSize, nullptr, numTests, modeNumParticles));
        }
        trailFunc(circles, radius, boxSize, square, numTests, modeNumParticles);
        for (int a = 0; a < NUM_THREADS; a++) {
            threads[a].join();
        }
 ...
 */
bool SquareHeuristic::isContained(const Point& circle, double radius, Point square, double sideLength, double regionLength, bool advancedAlgorithm) {
    double halfSide = sideLength / 2.0;
    if (advancedAlgorithm) {
        double largerHalfSide = halfSide + radius;
        ...
        if (Point::getDistance(newCircle, Point(square.x - halfSide, square.y - halfSide)) < radius) return true;
        if (Point::getDistance(newCircle, Point(square.x + halfSide, square.y - halfSide)) < radius) return true;
        if (Point::getDistance(newCircle, Point(square.x + halfSide, square.y + halfSide)) < radius) return true;
        if (Point::getDistance(newCircle, Point(square.x - halfSide, square.y + halfSide)) < radius) return true;
...
std::vector<Point> SquareHeuristic::getCircles(const std::string& filename) {
    ...
    radius = std::stod(str.substr(0, commaPos)) / 2.0;
    str = str.substr(commaPos + 1);
    commaPos = str.find_first_of(SEPARATING_CHARACTER);
